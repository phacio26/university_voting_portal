...
<div class="d-flex justify-content-between align-items-start flex-wrap gap-2 mb-3">
    <div>
        <h1 class="h4 portal-heading mb-1">Review Your Ballot</h1>
        <p class="portal-muted mb-0">Confirm your choices before final submission.</p>
    </div>
</div>
...
<div class="portal-card p-3 p-md-4">
    @if(empty($voteDetails))
        <div class="alert alert-warning mb-0">
            No votes available yet. <a href="{{ route('student.voting.start') }}">Start voting</a>.
        </div>
    @else
        ...
        <div class="d-flex justify-content-between flex-wrap gap-2">
            <a href="{{ route('student.voting.check') }}" class="btn btn-outline-secondary">
                Continue Voting
            </a>
            @if(count($voteDetails) == $positions->count())
                <form action="{{ route('student.voting.submit') }}" method="POST">
                    ...
                    <button class="btn btn-success">
                        <i class="fas fa-paper-plane me-1"></i>Submit All Votes
                    </button>
                </form>
            @else
                <span class="text-muted">Complete all positions before submitting.</span>
            @endif
        </div>
    @endif
</div>
